...
- (CGFloat)tableView:(NSTableView *)tableView heightOfRow:(NSInteger)row {
	CGFloat cellHeight = [(MFLogViewerTableCell*)[self preparedCellAtColumn:0 row:row] heightForCellInWidth:[self visibleRect].size.width];
...
- (void)copy:(id)sender {
	...
	for(i=0; i < [indexes count]; i++) {
		[stringData appendString:[NSString stringWithFormat:@"%@ %@\n", headerStringForASLMessageDict([self.logMessages objectAtIndex:index]),[[self.logMessages objectAtIndex:index] objectForKey:kMFLogKeyMessage]]];
