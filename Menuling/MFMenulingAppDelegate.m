...
- (void)connectToServer:(id)sender {
	...
	if ([[qmController window] isVisible] && [NSApp keyWindow] != [qmController window]) {
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
	...
	[menu addItemWithTitle: @"Connect to Server ..." action:@selector(connectToServer:) keyEquivalent:@""];
	...
	[menu addItemWithTitle:@"Open Configuration ..." action:@selector(openConfiguration:) keyEquivalent:@""];
...
- (NSError*)application:(NSApplication *)app willPresentError:(NSError*)error {
	...
	NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
							  ...
							  nil];
	MFError *newError = [MFError errorWithDomain:kMFErrorDomain code:kMFErrorCodeMountFaliure userInfo:userInfo];
	return newError;
}
