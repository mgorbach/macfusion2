...
- (NSString *)localizedDescription {
	...
	if (![[self userInfo] objectForKey: NSLocalizedDescriptionKey ] && paramName) {
		...
		if ([self code] == kMFErrorCodeMissingParameter) {
			return [NSString stringWithFormat: @"Missing required value for %@",paramName ];
		}
		if ([self code] == kMFErrorCodeInvalidParameterValue) {
			return [NSString stringWithFormat: @"Invalid value for %@\n%@",[[self userInfo] objectForKey: kMFErrorParameterKey],faliureReason ? faliureReason : @"" ];
		}
	}
...
+ (MFError *)errorWithErrorCode:(NSInteger)code description:(NSString *)description {
	return [MFError errorWithDomain:kMFErrorDomain code:code userInfo: 
