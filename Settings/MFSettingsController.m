...
- (void)newFSPopupClicked:(id)sender {
	...
	if (selectionIndex != NSNotFound) {
		...
		[self resizeWindowForContent];
		NSInteger editReturn = [MFEditingController editFilesystem:fs onWindow:[filesystemTableView window]];
...
- (NSArray *)selectedFilesystems {
	...
	NSArray* selectedFilesystems = [[filesystemArrayController arrangedObjects] objectsAtIndexes:[filesystemTableView selectedRowIndexes]];
	if (clickedFS) {
		...
		return [[filesystemArrayController arrangedObjects] objectsAtIndexes:[filesystemTableView selectedRowIndexes]];
	}
}
