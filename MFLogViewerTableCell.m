...
- (NSAttributedString *)headerForMessage:(NSDictionary *)messageDict controlView:(NSView *)controlView;
- (NSAttributedString *)textForMessage:(NSDictionary *)messageDict controlView:(NSView *)controlView;
@end
...
- (void)drawInteriorWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
	if (![controlView inLiveResize]) {
		...
		NSAttributedString *headerText = [self headerForMessage:messageDict controlView:controlView];
		NSAttributedString *messageText = [self textForMessage:messageDict controlView: controlView];
...
- (NSAttributedString *)headerForMessage:(NSDictionary *)messageDict controlView:(NSView *)controlView {
	...
	NSAttributedString *headerAttributed = [[NSAttributedString alloc] initWithString:header attributes:[self headerAttributesWithControlView:controlView]];
	return headerAttributed;
}

- (NSAttributedString *)textForMessage:(NSDictionary *)messageDict controlView:(NSView  *)controlView {
	NSString *message = [messageDict objectForKey:kMFLogKeyMessage];
	NSAttributedString *messageAttributed = [[NSAttributedString alloc] initWithString:message attributes:[self textAttributesWithControlView:controlView]];
...
- (NSDictionary *)textAttributesWithControlView:(NSView *)controlView {
	BOOL current = ([[controlView window] firstResponder] == controlView && [[controlView window] isKeyWindow]);
...
- (CGFloat)heightForCellInWidth:(CGFloat)width {
	...
	if ([_heightCache objectForKey: key]) {
		...
		for (NSString *line in lines) {
			NSSize lineSize = [[line stringByTrimmingCharactersInSet: [NSCharacterSet newlineCharacterSet]] sizeWithAttributes: attributes];
			float numLines = ceilf(lineSize.width / width);
			textHeight += lineSize.height * numLines;
		}
		
		[_heightCache setObject: [NSNumber numberWithFloat: textHeight] forKey: key];
		return textHeight;
	}
}
