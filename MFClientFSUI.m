...
- (NSViewController *)viewControllerForKey:(NSString *)key {
	...
	if (key == kMFUIMacfusionAdvancedViewKey) {
		NSViewController *macfusionAdvancedController = [[MFAdvancedViewController alloc] initWithNibName:@"macfusionAdvancedView"bundle: [NSBundle bundleForClass: [self class]]];
...
- (NSArray *)configurationViewControllers {
	if (!viewControllers) {
		...
		[configurationViewControllers makeObjectsPerformSelector:@selector(setRepresentedObject:) withObject:self];
...
- (NSView *)editingView {
	...
	if (!editingTabView) {
		...
		[tabView setFont:[NSFont systemFontOfSize:[NSFont systemFontSizeForControlSize: NSSmallControlSize]]];
